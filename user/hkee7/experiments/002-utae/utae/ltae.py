...
class LTAE2d(nn.Module):
    ...
    def forward(self, x, batch_positions=None, pad_mask=None, return_comp=False):
        ...
        if pad_mask is not None:
            ...
            pad_mask = (
                pad_mask.permute(0, 2, 3, 1).contiguous().reshape(sz_b * h * w, seq_len)
            )

        out = x.permute(0, 3, 4, 1, 2).contiguous().reshape(sz_b * h * w, seq_len, d)
        ...
        if self.positional_encoder is not None:
            ...
            bp = bp.permute(0, 2, 3, 1).contiguous().reshape(sz_b * h * w, seq_len)
            out = out + self.positional_encoder(bp)

        out, attn = self.attention_heads(out, pad_mask=pad_mask)

        out = (
            out.permute(1, 0, 2).contiguous().reshape(sz_b * h * w, -1)
        )  # Concatenate heads
        out = self.dropout(self.mlp(out))
        out = self.out_norm(out) if self.out_norm is not None else out
        out = out.reshape(sz_b, h, w, -1).permute(0, 3, 1, 2)

        attn = attn.reshape(self.n_head, sz_b, h, w, seq_len).permute(
            0, 1, 4, 2, 3
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, v, pad_mask=None, return_comp=False):
        ...
        q = torch.stack([self.Q for _ in range(sz_b)], dim=1).reshape(
            -1, d_k
        )  # (n*b) x d_k

        k = self.fc1_k(v).reshape(sz_b, seq_len, n_head, d_k)
        k = (
            k.permute(2, 0, 1, 3).contiguous().reshape(-1, seq_len, d_k)
        )  # (n*b) x lk x dk
        ...
        v = torch.stack(v.split(v.shape[-1] // n_head, dim=-1)).reshape(
