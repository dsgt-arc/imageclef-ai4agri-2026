...
class SpectralViTPixel(nn.Module):
    def __init__(self, num_bands, num_classes,patch_size=128, d_model=128, depth=4, nhead=4):
        super().__init__()
    ...
    def forward(self, x):
        ...
        cols = self.col_embed.unsqueeze(0).expand(H, W, -1) 
        pos = (rows + cols).reshape(H*W, -1) # (HW, d_model)
