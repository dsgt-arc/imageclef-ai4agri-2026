...
from utils import accuracy_exact, accuracy_pm1, ordinal_confidence, ordinal_loss, ordinal_predict, valid_mask
...
FIXED_TEMPORAL_COORDS = torch.tensor([
    [2018, date_to_doy(28, 1, 2018)],
    [2018, date_to_doy(18, 4, 2018)],
    [2018, date_to_doy(26, 8, 2018)],
    [2018, date_to_doy(20, 9, 2018)],
], dtype=torch.float32)
...
class ChunkedDataset(Dataset):
    def __init__(self, mode, cache_size=8):
        ...
        if os.path.exists(meta_path):
            with open(meta_path) as handle:
                chunk_lengths = json.load(handle)
        else:
            ...
            with open(meta_path, 'w') as handle:
                json.dump(chunk_lengths, handle)
    ...
    def _aggregate_seasons(self, data):
        data = data[:, PRITHVI_BAND_INDICES].float()
    ...
    def __getitem__(self, idx):
        ...
        raw = payload['data'][patch_idx].to(torch.float32)
        ...
        prithvi_data = self._aggregate_seasons(raw)
        unet_data = self._add_indices(raw)
...
class MetricsCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        print(
            f"  train loss: {loss.item():.4f} | gt: {loss_gt.item():.4f} | pseudo: {loss_pseudo.item():.4f} | "
            f"pseudo %: {pseudo_pct.item() * 100:.2f}% | conf mean: {conf_mean.item():.4f}"
        )
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        print(
            f"  val loss: {loss.item():.4f} | val acc ±1: {acc_pm1.item():.4f} | val acc exact: {accuracy_exact.item():.4f}"
        )
        self._plot()
...
class OrdinalSegmentationTask(SemanticSegmentationTask):
    ...
    def configure_optimizers(self):
        ...
        cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.epochs - 3, eta_min=1e-7)
        ...
        if self.teacher is not None:
            ...
            if pseudo_mask.any():
                ...
                if loss_pseudo is not None and not torch.isnan(loss_pseudo):
                    ...
                    print('Invalid loss (NaN or None) encountered in training step.', batch_idx)
                    loss = loss_gt
            else:
                loss = loss_gt
